...
public class ABEEK_Graduation extends Grade_Graduation
{
	...
	private ArrayList <Integer> abeeklist = new ArrayList <Integer> ();
	
	public ArrayList<Integer> getAbeeklist() {
		return abeeklist;
	}
	public void setAbeeklist(ArrayList<Integer> abeeklist) {
		this.abeeklist = abeeklist;
	}
	...
	{	
		ABEEK_Data a = new ABEEK_Data();
		abeeklist = a.getRequire(Filename);
		basic_literacy_point = abeeklist.get(0);
		major_base_point = abeeklist.get(1);
		engineering_major_point = abeeklist.get(2);
	...
	{
		ABEEK_Data a = new ABEEK_Data();
		a.store_abeek_data(abeeklist, Filename);
	...
	public void modified_ABEEK_graduation_requirement(int selection, int new_grade_point) // ABEEK졸업요건 수정하기
	{
		if(selection == 1)
		{
			...
			abeeklist.add(0, new_grade_point);
		}
		else if(selection == 2)
		{
			major_base_point = new_grade_point;
			abeeklist.add(1, new_grade_point);
		}
		else
		{
			engineering_major_point = new_grade_point;
			abeeklist.add(2, new_grade_point);
		}
	}
} 
