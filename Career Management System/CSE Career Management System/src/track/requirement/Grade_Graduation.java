...
public class Grade_Graduation 
{
	...
	public void getRequirements(String Filename) // 학점 졸업요건 가져오기
	{	
		Grade_Data g = new Grade_Data();
		gradelist = g.getRequire(Filename);
		total_grade_point = gradelist.get(0);
		major_grade_point = gradelist.get(1);
		liberalart_grade_point = gradelist.get(2);
	...
	public void store_grade_graduation_requirement(String Filename) // 학점 졸업요건 수정하기
	{
		Grade_Data g = new Grade_Data();
		g.store_grade_data(gradelist, Filename);
	...
	public void modified_grade_graduation_requirement(int selection, int new_grade_point) // 학점 졸업요건 수정하기
	{
		if(selection == 1)
		{
			total_grade_point = new_grade_point;
			gradelist.add(0, new_grade_point);
		}
		else if(selection == 2)
		{
			major_grade_point = new_grade_point;
			gradelist.add(1, new_grade_point);
		}
		else
		{
			liberalart_grade_point = new_grade_point;
			gradelist.add(2, new_grade_point);
		}
	}
}
